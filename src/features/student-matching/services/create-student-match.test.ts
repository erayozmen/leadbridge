...
type Options={vr?:{id:string;matched:boolean}|null;registration?:{id:string;matched:boolean}|null;error?:unknown};
function deps(o:Options={}) { const tx={findVr:vi.fn(async()=>o.vr===undefined?{id:"vr_1",matched:false}:o.vr),findRegistration:vi.fn(async()=>o.registration===undefined?{id:"reg_1",matched:false}:o.registration),create:vi.fn(async(data:{matchedByUserId:string;matchedAt:Date})=>{if(o.error)throw o.error;return{id:"match_1",matchedAt:data.matchedAt};})}; const d:CreateStudentMatchDependencies={requireAdmin:vi.fn(async()=>admin),runTransaction:vi.fn(async cb=>cb(tx))}; return{d,tx}; }
