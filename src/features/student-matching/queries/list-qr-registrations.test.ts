...
describe("listQrRegistrations",()=>{
  ...
  it("selects minimum removal IDs and linked VR without token fields",async()=>{const d=deps();await listQrRegistrations({}, {},d);const args=vi.mocked(d.findMany).mock.calls[0][0];expect(args.select).toMatchObject({id:true,studentMatch:{select:{id:true,vrRecord:{select:{id:true}}}}});expect(args.select).not.toHaveProperty("qrCode.tokenHash");expect(JSON.stringify(args.select)).not.toContain("tokenHash");});
});
