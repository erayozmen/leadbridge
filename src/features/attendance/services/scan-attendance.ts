...
const failure = (
  code: Exclude<ScanAttendanceResult, { ok: true }>["code"],
  message: string,
...
export async function scanAttendance(
  ...
  if (!parsed.success)
    return failure("INVALID_TOKEN", "QR bağlantısı geçerli değil.");
  ...
  try {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (!qr?.qrRegistration)
        return failure("NOT_FOUND", "Bu QR için öğrenci kaydı bulunamadı.");
      if (
        qr.eventId !== event.id ||
        qr.qrRegistration.eventId !== event.id
      )
        return failure("NOT_AVAILABLE", "QR farklı bir etkinliğe ait.");
      if (qr.archivedAt || qr.status !== QrCodeStatus.USED)
        return failure(
          "NOT_AVAILABLE",
          "QR katılım işlemi için uygun değil.",
        );
      if (qr.qrRegistration.attendedEvent)
        return failure(
          "ALREADY_ATTENDED",
          "Öğrenci daha önce katıldı olarak işaretlenmiş.",
        );

      const updated = await tx.qrRegistration.updateMany({
        where: {
          id: qr.qrRegistration.id,
          eventId: event.id,
          attendedEvent: false,
        },
        data: {
      ...
      if (updated.count !== 1)
        return failure(
          "ALREADY_ATTENDED",
          "Katılım başka bir görevli tarafından işlendi.",
        );
      return {
        ok: true as const,
        studentName: `${qr.qrRegistration.firstName} ${qr.qrRegistration.lastName}`,
      };
    });
  } catch {
    return failure("FAILED", "Katılım işlemi tamamlanamadı.");
  }
}
