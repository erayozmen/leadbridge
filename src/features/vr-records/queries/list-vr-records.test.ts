...
describe("listVrRecords", () => {
  ...
  it("preserves pagination", async () => { const deps = dependencies(); const result = await listVrRecords({ page: 3 }, deps); expect(deps.findMany).toHaveBeenCalledWith(expect.objectContaining({ skip: 50, take: VR_RECORDS_PAGE_SIZE })); expect(result).toMatchObject({ page: 3, pageCount: 2, total: 45 }); });
