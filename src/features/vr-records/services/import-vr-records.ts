...
export async function importVrRecords(file:File,commit=false):Promise<VrImportResult>{
  ...
  const existing=await prisma.vrRecord.findMany({where:{eventId:event.id,OR:rows.map(row=>({firstName:row.firstName,lastName:row.lastName,schoolId:row.schoolId,phone:row.phone||null}))},select:{firstName:true,lastName:true,schoolId:true,phone:true}});
