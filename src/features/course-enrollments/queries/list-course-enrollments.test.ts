...
describe("listCourseEnrollments", () => {
  ...
  it("paginates by twenty", async () => { const state = deps(); await listCourseEnrollments({ page: 3 }, state); expect(state.findMany).toHaveBeenCalledWith(expect.objectContaining({ skip: 40, take: COURSE_ENROLLMENTS_PAGE_SIZE })); });
