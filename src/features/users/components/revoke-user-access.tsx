...
export function RevokeUserAccess({ userId, disabled }: { userId: string; disabled: boolean }) {
  ...
  return <form action={action} className="grid min-w-80 gap-2">
    ...
    <textarea id={`revoke-reason-${userId}`} name="reason" value={reason} onChange={(event) => setReason(event.target.value)} minLength={10} maxLength={500} required disabled={pending} className="min-h-20 rounded-md border bg-background px-3 py-2 text-sm" />
