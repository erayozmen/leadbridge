...
export default async function EventsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
    <PageHeader icon={CalendarDays} eyebrow={<StatusBadge status="ACTIVE" />} title="Etkinlikler" description="Etkinlikleri ve yaşam döngülerini yönetin." actions={<Button asChild><Link href="/dashboard/events/new">Yeni etkinlik</Link></Button>} />
    <form className="mt-6 grid gap-3 rounded-lg border border-white/80 bg-card/90 p-4 shadow-sm sm:grid-cols-[1fr_220px_auto]" method="get"><Input name="query" defaultValue={query} placeholder="Etkinlik ara" maxLength={120} /><select name="status" defaultValue={status ?? ""} className="h-10 rounded-md border bg-background px-3 text-sm"><option value="">Tüm durumlar</option>{Object.values(EventStatus).map((value) => <option key={value} value={value}>{value}</option>)}</select><Button type="submit">Filtrele</Button></form>
